...
int main() {
  ...
  string map_file_ = "/Users/srikanth.vidapanakal/CarND-Path-Planning-Project/data/highway_map.csv";
  ...
  h.onMessage([&map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
		...
		bool too_close_in_lane = false;
		bool safe_left  = true;
		bool safe_right = true;

		// Find ref velocity to use based on collision possibility
		...
		// Current car's location as reference
		double ref_x = car_x;
		double ref_y = car_y;
		...
		// transformation from map to car coordinates
		for (int i = 0; i < pts_x.size(); i++)
		{
			double shiftx = pts_x[i] - ref_x;
			double shifty = pts_y[i] - ref_y;

			pts_x[i] = (shiftx * cos(0 - ref_yaw) - shifty * sin(0 - ref_yaw));
			pts_y[i] = (shiftx * sin(0 - ref_yaw) + shifty * cos(0 - ref_yaw));
		}

		// Create a spline
		tk::spline s;

		// set the XY points to the spline
		...
		for (int i = 0; i < 50; i++)
		{
			...
			vector<double> my_xy = getXY(next_s, next_d, map_waypoints_s, map_waypoints_x, map_waypoints_y);

			next_x_vals.push_back(my_xy[0]);
			next_y_vals.push_back(my_xy[1]);
